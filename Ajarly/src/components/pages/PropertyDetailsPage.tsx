...
export function PropertyDetailsPage({
  ...
  const t =
    translations[language]?.propertyDetails || translations.en.propertyDetails;
  ...
  useEffect(() => {
    ...
    if (bookingIntent && property) {
      try {
        ...
        // Check if this is the same property
        if (intent.propertyId === propertyId) {
          console.log("Restoring booking intent:", intent);

          if (intent.checkIn) {
            setCheckIn(new Date(intent.checkIn));
          }
          if (intent.checkOut) {
            setCheckOut(new Date(intent.checkOut));
          }
          if (intent.guests) {
  ...
  const loadPropertyData = async () => {
    try {
      ...
      try {
        ...
        setImages(
          imagesData.sort((a: any, b: any) => a.imageOrder - b.imageOrder)
        );
      } catch (imgError) {
      ...
      try {
        ...
        // Check if response has content array or is already an array
  ...
  const checkAvailability = async () => {
    ...
    try {
      ...
      // If 403, user needs to login, but don't block booking
      if (err.status === 403 || err.status === 401) {
        console.log("User not authenticated, skipping availability check");
      }
      // Don't show availability errors, they're not critical
      setAvailabilityError(null);
    } finally {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              ...
              <p className="text-gray-600">
                {property.bedrooms} {language === "ar" ? "غرف نوم" : "bedrooms"}{" "}
                •{property.bathrooms}{" "}
            ...
            <div>
              <h3 className="text-xl font-semibold text-[#2B2B2B] mb-4">
                {language === "ar" ? "عن هذا العقار" : "About this place"}
              </h3>
            ...
            <div className="sticky top-28 border border-gray-200 rounded-2xl p-6 shadow-xl">
              ...
              >
                {checkingAvailability ? (
                  ...
                  t.reserve || (language === "ar" ? "احجز" : "Reserve")
                )}
              </Button>
